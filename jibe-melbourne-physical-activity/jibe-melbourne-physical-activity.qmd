...
Marginal metabolic equivalent houts per week (mMET hours/week; `mmet`) are calculated as the sum of hours spent walking for recreation, doing moderate exercise and doing vigorous exercise, with each respectively multiplied by the metabolic equivalent of these tasks (METs) minus one (already in MMETs in code).
...
pa_data <- nhs  %>%
  ...
  mutate(
    ...
    mutate(
        ...
        mmet_hours_per_week_total=walk_recreation_min/60* MMET_WALKING +
...
## Summary statistics of mmets for prediction model (moderate recreational, vigorous recreational and walkint for recreation)
mmets_1 <- pa_data %>%
  summarise(
    min_mmet = min(mmet_hours_per_week, na.rm = TRUE),
    q1_mmet = quantile(mmet_hours_per_week, 0.25, na.rm = TRUE),
    median_mmet = median(mmet_hours_per_week, na.rm = TRUE),
    mean_mmet = mean(mmet_hours_per_week, na.rm = TRUE),
    q3_mmet = quantile(mmet_hours_per_week, 0.75, na.rm = TRUE),
    max_mmet = max(mmet_hours_per_week, na.rm = TRUE)
  ) %>%
  mutate(type="NHS includes moderate, vig for recreation and walking for recreation")

mmets_1 

## Summary statistics of mmets for comparison with Meta analysis (moderate recreational, vigorous recreational and walking for recreation and transport)
mmets_2 <- pa_data %>%
  summarise(
    min_mmet = min(mmet_hours_per_week_total, na.rm = TRUE),
    q1_mmet = quantile(mmet_hours_per_week_total, 0.25, na.rm = TRUE),
    median_mmet = median(mmet_hours_per_week_total, na.rm = TRUE),
    mean_mmet = mean(mmet_hours_per_week_total, na.rm = TRUE),
    q3_mmet = quantile(mmet_hours_per_week_total, 0.75, na.rm = TRUE),
    max_mmet = max(mmet_hours_per_week_total, na.rm = TRUE)
  ) %>%
  mutate(type="NHS includes moderate, vig for recreation and walking for recreation")
